...
| 2026-06-12 | Assignment IS Console login showed `invalid_callback` / `callback.not.match` | Assignment IS was using the default `https://localhost:9443/console` runtime URL instead of the Lab 16 local `https://localhost/console` pattern | Mount an Assignment IS `deployment.toml` with `base_path = "https://localhost"` and HTTPS `proxyPort = 443`; document not to open `https://localhost:9443/console` | Verified |
